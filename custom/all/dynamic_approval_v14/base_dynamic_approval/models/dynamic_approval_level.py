...
class DynamicApprovalLevel(models.Model):
    ...
    approval_id = fields.Many2one(comodel_name='dynamic.approval',)
    sequence = fields.Integer(default=1,)
    user_id = fields.Many2one(comodel_name='res.users', string='Approver User',)
    group_id = fields.Many2one(comodel_name='res.groups', string='Approver Group')
    company_id = fields.Many2one(comodel_name='res.company', readonly=True, related='approval_id.company_id', store=True,)
    apply_domain = fields.Boolean(string="Apply Domain")
    domain = fields.Char(string='Domain')
    model = fields.Char(related='approval_id.model_id.model')
