...
class product_template(models.Model):
    ...
    @api.depends('pre_order_date', 'max_qty')
    def expired_text_vissible(self):
        for record in self:
            if record.outgoing_pre >= 0:
                qty = record.outgoing_pre
            else:
                qty = - record.outgoing_pre

            if record.is_preorder_type:

                if record.pre_order_date and record.pre_order_date >= datetime.today().date():
                    if record.max_qty and record.max_qty > qty:
                        _logger.warning('qty' + str(qty) + ' max' + str(record.max_qty) + 'false')
                        record.visible = False
                        record.inventory_availability = 'never'
                    else:
                        _logger.warning('qty' + str(qty) + ' max' + str(record.max_qty) + 'true')
                        record.visible = True
                        record.inventory_availability = 'always'

                else:

                    _logger.warning('qty' + str(qty) + ' max' + str(record.max_qty) + 'true')
                    record.visible = True
                    record.inventory_availability = 'threshold'
            else:
                record.visible = True

    outgoing_pre = fields.Float('Outgoing Pre', compute='_compute_func_pre', store=False)

    def _compute_func_pre(self):
        for rec in self:
            product = self.env['product.product'].search([('product_tmpl_id','=',rec.id)],limit=1)
            website= self.env['website'].search([],limit=1)
            rec.outgoing_pre = product.with_context(location=website.warehouse_id.lot_stock_id.id).outgoing_qty
    ...
    payment_type = fields.Selection([('complete', 'Complete Payment'), ('percent', 'Percent Payment')],
                                    string="Pre-order Payment Type", default="complete", required=True,
    ...
    percentage = fields.Integer(string="Percent Payment For Pre-order",
                                help="If 'payment Method Type' is 'Percent Payment' then set the percentage for percent Payment.",
                                default=get_default_percentage_value)
    minimum_qty = fields.Float(string="Allow preorder when quantity Less than or Equal",
                               help="Set the minimum quantity of the product when it goes to the pre-order product.",
                               default=get_default_minimum_qty)
    max_qty = fields.Float(string="Maximum Quantity",
                           help="Set maximum quantity will be pre-ordered.")
    ...
    wk_override_pre_order_default = fields.Boolean(
        string="Override Default Pre-order Configuration",
        help="Set the maximum quantity of pre-order that can be ordered.")
    ...
    def write(self, vals):
        ...
        for res in self:
            if vals.get('is_preorder_type') or (not vals.get('is_preorder_type') and res.is_preorder_type):
                if (not vals.get('wk_override_pre_order_default', True)) or (
                        not vals.get('wk_override_pre_order_default') and not res.wk_override_pre_order_default):
                    vals.update({
            ...
            if vals.get('payment_type'):
                if vals.get('payment_type') == 'percent':
                    if vals.get('percentage', False):
                        ...
                        raise UserError(
                            _("Percent value lies between 1 to 100"))
            elif res.payment_type == 'percent' and vals.get('percentage', False) and (
                    vals.get('percentage') > 99 or vals.get('percentage') < 1):
    ...
    def create(self, vals):
        if vals.get("wk_override_pre_order_default"):
            if vals.get('payment_type', False) and vals.get('payment_type') == 'percent' and (
                    vals.get('percentage') > 99 or vals.get('percentage') < 1):
                raise UserError(_("Percent value lies between 1 to 100"))
            if vals.get("minimum_qty", False) and vals.get("minimum_qty") < 0:
                raise UserError(
            ...
            if vals.get("min_order_qty", False) and vals.get("min_order_qty") < 1:
                raise UserError(
                    _("Minimum order quantity of a pre-order product must be greater than zero"))
            if vals.get("min_order_qty", False) and vals.get("max_order_qty") and vals.get("min_order_qty") > vals.get(
                    "max_order_qty"):
                raise UserError(
                    _("Minimum order quantity of a pre-order product must be less than maximum order quantity"))
...
class SaleOrder(models.Model):
    ...
    preorder_notify = fields.Boolean(string="Pre-Order Notification", compute=set_sale_order_preorder_notify,
    ...
    def action_confirm(self):
        for order in self:
            if order.preorder_payment_state == 'partial':
                raise UserError(_(
                    "You cannot confirm this sale order as it contains a partial pre order product whose complete payment is not done."))
    ...
    def _website_product_id_change(self, order_id, product_id, qty=0):
        ...
        if self.env['website'].get_pre_order(product_obj):
            ...
            if res.get('product_uom_qty') > stock_value:
                ...
                res['pre_order_qty'] = (res.get('product_uom_qty') - stock_value) if stock_value > 0 else res.get(
                    'product_uom_qty')
