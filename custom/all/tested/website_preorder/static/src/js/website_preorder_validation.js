...
odoo.define('website_preorder.website_preorder_validation', function(require) {
    ...
    $(document).ready(function() {
        ...
        $(".oe_website_sale a[href='/shop/checkout?express=1']").on('click', function(ev) {
            var $checkout = $(this);
            ev.preventDefault();
            ajax.jsonRpc("/user/validation", 'call', {})
                 .then(function(result) {
                    if(result){

                           if ($(".pre-order-alert")[0]){
                           ...
                           $('.pre-order-alert').each(function(){
                             ev.preventDefault();
                                ...
                                ajax.jsonRpc("/max/quantity/validation", 'call', {
                    ...
                    'set_value': value,
                        ...
                        $('.max-alert').show();
                        setTimeout(function() {$('.max-alert').hide()}, 12000);
                    }
                                else{
                    ...
                    'set_value': value,
                        ...
                        setTimeout(function() {$this.hide()}, 12000);
                                ...
                                ajax.jsonRpc("/stock/validation", 'call', {
                                       'product_id': preorder_product_id,
                                       'set_value': value,
                                  }).then(function(result) {
                                if(result){
                                     ...
                                     $('.stock-alert').show();
                                       setTimeout(function() {$('.stock-alert').hide()}, 12000);
                                        }
                                else{
                                       window.location.href = window.location.origin+'/shop/checkout'
                                    }
