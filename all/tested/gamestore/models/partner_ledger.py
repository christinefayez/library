...
class ReportPartnerLedger(models.AbstractModel):
        ...
        for res in self._cr.dictfetchall():
            ...
            if key == 'sum':
                if not company_currency.is_zero(res['debit']) or not company_currency.is_zero(res['credit']) \
                        or not company_currency.is_zero(res['amount_currency']):
    ...
    def _get_report_line_partner(self, options, partner, initial_balance, debit, credit, balance, amount_currency,
        ...
        if self.user_has_groups('base.group_multi_currency'):
            try:
               ...
               columns.append({'name': self.format_value(amount_currency, currency=currency_id, blank_if_zero=False),
                              'class': 'number'})
            except:
                pass
        ...
        try:
            ...
            return {
                ...
                'level': 2,
                'trust': partner.trust,
                'unfoldable': not company_currency.is_zero(debit) or not company_currency.is_zero(credit)or not currency_id.is_zero(amount_currency),
            ...
            return {
                ...
                'level': 2,
                'trust': partner.trust,
                # 'unfoldable': not company_currency.is_zero(debit) or not company_currency.is_zero(
                #     credit) or not currency_id.is_zero(amount_currency),
                'unfolded': 'partner_%s' % partner.id in options['unfolded_lines'] or unfold_all,
                'colspan': 6,
            }
    ...
    def _get_partner_ledger_lines(self, options, line_id=None):
        ...
        total_initial_balance = total_debit = total_credit = total_balance = total_amount = 0.0
        for partner, results in partners_results:
            ...
            currency = partner_sum.get('currency', 0.0)
