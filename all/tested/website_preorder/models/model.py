...
class Website(models.Model):
    ...
    def min_preorder_checkout_validate(self):
        ...
        if order:
            ...
            for line in order_lines:
                ...
                vals2 = False if self.get_pre_order(
                    line.product_id) and line.product_uom_qty > stock_value and line.product_uom_qty < self.get_preorder_min_qty(
                    line.product_id) else True
                if not vals2:
                    break
            else:
                return True
    ...
    @api.model
    def max_preorder_checkout_validate(self, uid):
        order = self.sale_get_order()
        if order:
            order_lines = order.website_order_line
            for line in order_lines:
                if line.product_id.product_tmpl_id.is_preorder_type:
                    user = self.env['res.users'].search([('id', '=', uid)])
                    orders = self.env['sale.order.line'].search([('order_partner_id', '=', user.partner_id.id),
                                                                 ('product_id', '=', line.product_id.id),
                                                                 ('is_preorder', '=', True),('state','not in',['draft','sent'])])
                    old_qty = 0
                    max_qty = line.product_id.max_order_qty
                    for order in orders:
                        old_qty += order.product_uom_qty
                    if (max_qty - old_qty) < line.product_uom_qty:
                        return False
    ...
    @api.model
    def stock_checkout_validate(self):
        order = self.sale_get_order()
        if order:
            order_lines = order.website_order_line
            for line in order_lines:
                if line.product_id.product_tmpl_id.is_preorder_type:
                    orders = self.env['sale.order.line'].search([
                                                                 ...
                                                                 ('is_preorder', '=', True),
                                                                 ('order_id','!=',order.id)])
                    # print(order.id,'test6')
                    old_qty = line.product_id.outgoing_pre
    ...
    def get_pre_order(self, product):
        res = self.get_preorder_config_settings_values()
        if product.sudo().virtual_available >= 0:
            qty = product.sudo().virtual_available
        else:
            qty = - product.sudo().virtual_available
        if res and product.is_preorder_type and product.pre_order_date >= datetime.today().date() and product.max_qty > qty:
            return True
        return False
