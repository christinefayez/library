...
class WebsiteSale(WebsiteSale):
    ...
    @http.route(['/user/validation'], type='json', methods=['POST'], auth="public", website=True)
    def user_validation(self, **post):
    ...
    @http.route(['/max/quantity/validation'], type='json', methods=['POST'], auth="public", website=True)
    def max_quantity_validation(self, product_id, set_value, **post):
        product = request.env['product.product'].browse(int(product_id))

        user = request.env['res.users'].sudo().search([('id', '=', request.session.uid)])
        orders = request.env['sale.order.line'].sudo().search([('order_partner_id', '=', user.partner_id.id),
                                                     ('product_id', '=', product.id),
                                                     ('is_preorder', '=', True),('state','not in',['draft','sent'])])
        # print(request.session.uid)
        old_qty = 0
        max_qty = product.max_order_qty
        for order in orders:
            old_qty += order.product_uom_qty
        if set_value and (max_qty - old_qty) < set_value:
            return True
        return False

    @http.route(['/stock/validation'], type='json', methods=['POST'], auth="public", website=True)
    def stock_quantity_validation(self, product_id, set_value, **post):
        product = request.env['product.product'].browse(int(product_id))
    ...
    def checkout(self, **post):
        ...
        check_max = (request.website.max_preorder_checkout_validate(request.session.uid))
        check_stock = request.website.stock_checkout_validate()
        # print(check_max)

        if request.session.uid:

            if check and check_max and check_stock:
