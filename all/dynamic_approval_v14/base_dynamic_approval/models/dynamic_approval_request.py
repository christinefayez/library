...
class DynamicApprovalRequest(models.Model):
    ...
    approval_states = fields.Char(string='State Approval', compute='get_dynamic_approve_state_from', store=True)
    last_reminder_date = fields.Datetime()

    @api.depends('dynamic_approval_id', 'dynamic_approval_id.state_from')
    def get_dynamic_approve_state_from(self):
        for rec in self:
            if rec.dynamic_approval_id:
                if rec.dynamic_approval_id.state_from == 'draft':
                    rec.approval_states = 'Quotation'
                elif rec.dynamic_approval_id.state_from == 'otr_approval':
                    rec.approval_states = 'Sales Order'
                else:
                    rec.approval_states = False
            else:
                rec.approval_states = False
        ...
        for record in self:
            if record.dynamic_approval_id and record.dynamic_approval_id.reminder_pending_approver_email_template_id:
                for user in record.get_approve_user():
                    ...
                    email = record.dynamic_approval_id.reminder_pending_approver_email_template_id.with_context(
                        name_to=user.name, user_lang=user.lang).send_mail(
                        self.res_id, email_values=email_values, force_send=True)

                    self.env['mail.mail'].search([('id', '=', email)]).send()
